...
#+BEGIN_SRC python
  .add_node(settings=[], type="", use_transform=False)
#+END_SRC
** Add a node to the active tree
#+BEGIN_SRC python
  .add_search(settings=[], type="", use_transform=False, node_item='<UNKNOWN ENUM>')
#+END_SRC
** Add a node to the active tree and link to an existing socket
#+BEGIN_SRC python
  .add_and_link_node(settings=[], type="", use_transform=False, link_socket_index=0)
#+END_SRC
** Add a reroute node
#+BEGIN_SRC python
